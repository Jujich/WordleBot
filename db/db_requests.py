from db.classes import User, Settings, Daily, Group
...
async def insert_group(group_id: int, admin_id: int) -> bool:
    session = Session(engine)
    try:
        group = Group(
            tgId=group_id,
...
async def check_group_exists(group_id: int) -> Group | bool:
    session = Session(engine)
    try:
        group = session.query(Group).filter(Group.tgId == group_id).first()
        session.close()
        return group
    except:
        session.close()
